...
public class InlineCalls extends ExpExpVisitor<Type> {
    ...
    public Expression checkType(Expression exp, Type type) {
        boolean incompatible;
        Expression converted;
        Method amethod;
        Type expType = exp.getType();
        if ((type instanceof ClassType) && ((ClassType) type).isInterface() && expType.isSubtype(Compilation.typeProcedure) && !expType.isSubtype(type)) {
            if ((exp instanceof LambdaExp) && (amethod = ((ClassType) type).checkSingleAbstractMethod()) != null) {
                LambdaExp lexp = (LambdaExp) exp;
                ObjectExp oexp = new ObjectExp();
                oexp.setLocation(exp);
                oexp.supers = new Expression[]{new QuoteExp(type)};
                ...
                return visit((Expression) oexp, type);
            }
            incompatible = true;
        } else {
            ...
            incompatible = type != 0 && type.compare(expType) == -3;
            if (incompatible && (type instanceof TypeValue) && (converted = ((TypeValue) type).convertValue(exp)) != null) {
                return converted;
            }
        }
        if (incompatible) {
            Language language = this.comp.getLanguage();
            this.comp.error('w', "type " + language.formatType(expType) + " is incompatible with required type " + language.formatType(type), exp);
        }
        return exp;
    }
    ...
    public Expression visitLetExp(LetExp exp, Type required) {
        ...
        while (true) {
            ...
            if (typeSpecified && init0 != QuoteExp.undefined_exp) {
                dtype = decl.getType();
            }
    ...
    public Expression visitTryExp(TryExp exp, Type required) {
        ...
        return (Expression) super.visitTryExp(exp, (TryExp) required);
    }
