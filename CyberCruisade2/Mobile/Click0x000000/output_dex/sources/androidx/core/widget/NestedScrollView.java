...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    private void onNestedScrollInternal(int dyUnconsumed, int type, int[] consumed) {
        ...
        this.mChildHelper.dispatchNestedScroll(0, myConsumed, 0, myUnconsumed, (int[]) null, type, consumed);
    }
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        ...
        if (event.getAction() == 8 && !this.mIsBeingDragged) {
            ...
            if (vscroll != 0.0f) {
                ...
                if (newScrollY < 0) {
                    if (canOverScroll() && !MotionEventCompat.isFromSource(event, FragmentTransaction.TRANSIT_FRAGMENT_CLOSE)) {
                        canOverScroll = true;
                    }
                    if (canOverScroll) {
    ...
    public void fling(int velocityY) {
        if (getChildCount() > 0) {
            this.mScroller.fling(getScrollX(), getScrollY(), 0, velocityY, 0, 0, RtlSpacingHelper.UNDEFINED, ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED, 0, 0);
