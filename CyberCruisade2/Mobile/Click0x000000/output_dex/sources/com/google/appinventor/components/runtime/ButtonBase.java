...
public abstract class ButtonBase extends AndroidViewComponent implements View.OnClickListener, View.OnFocusChangeListener, View.OnLongClickListener, View.OnTouchListener, AccessibleComponent {
    ...
    public ButtonBase(ComponentContainer container) {
        ...
        android.widget.Button button = new android.widget.Button(container.$context());
        this.view = button;
        this.defaultButtonDrawable = button.getBackground();
        this.defaultColorStateList = button.getTextColors();
        defaultButtonMinWidth = KitkatUtil.getMinWidth(button);
        defaultButtonMinHeight = KitkatUtil.getMinHeight(button);
        if (Build.VERSION.SDK_INT >= 21) {
            this.defaultOutlineProvider = button.getOutlineProvider();
            this.defaultStateAnimator = button.getStateListAnimator();
        }
        container.$add(this);
        ...
        TextViewUtil.setFontTypeface(container.$form(), button, this.fontTypeface, this.bold, this.italic);
        FontSize(14.0f);
        Text("");
        TextColor(0);
        Shape(0);
    ...
    @DesignerProperty(defaultValue = Component.TYPEFACE_SANSSERIF, editorType = "textalignment")
    ...
    @DesignerProperty(defaultValue = Component.TYPEFACE_DEFAULT, editorType = "button_shape")
    ...
    private ColorStateList createRippleState() {
        ...
        int enabled_color = this.defaultColorStateList.getColorForState(this.view.getDrawableState(), R.attr.state_enabled);
