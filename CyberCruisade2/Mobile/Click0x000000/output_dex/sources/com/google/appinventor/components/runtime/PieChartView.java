...
public class PieChartView extends ChartView<PieEntry, IPieDataSet, PieData, PieChart, PieChartView> {
    ...
    public void resizePieRings() {
        int lastWidth = 0;
        int lastHeight = 0;
        float reductionFactor = ((this.pieHoleRadius / 100.0f) + 0.75f) / this.pieCharts.size();
    ...
    private void changePieChartRadius(PieChart pieChart, float newHoleRadius, boolean lastChart) {
        ...
        int i = this.pieHoleRadius;
        if (i == 0) {
            pieChart.setDrawHoleEnabled(false);
            return;
        }
        float delta = newHoleRadius - i;
        float setRadius = i * ((delta / 100.0f) + 1.0f);
    ...
    private void updatePieChartRingOffset(PieChart pieChart) {
        if (this.chart == pieChart) {
            ...
            float f = dpNeededHeight / 2.5f;
            this.bottomOffset = f;
            this.bottomOffset = Math.min(25.0f, f);
        }
        float dpNeededHeight2 = this.bottomOffset;
        pieChart.setExtraBottomOffset(dpNeededHeight2);
