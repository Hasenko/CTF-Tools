...
public class SmsBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if ((receivingEnabled == 2 || isRepl(context)) && !Texting.isRunning()) {
    ...
    private void sendNotification(Context context, String phone, String msg) {
        ...
        try {
            ...
            Notification note = new NotificationCompat$Builder(context).setSmallIcon(R.drawable.sym_call_incoming).setTicker(phone + " : " + msg).setWhen(System.currentTimeMillis()).setAutoCancel(true).setDefaults(1).setContentTitle("Sms from " + phone).setContentText(msg).setContentIntent(activity).setNumber(Texting.getCachedMsgCount()).build();
