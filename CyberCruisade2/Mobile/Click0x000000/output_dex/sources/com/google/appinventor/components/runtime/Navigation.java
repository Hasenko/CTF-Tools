...
public class Navigation extends AndroidNonvisibleComponent implements Component {
    ...
    public void performRequest(GeoPoint start, GeoPoint end, TransportMethod method) throws IOException, JSONException {
        ...
        if (connection.getResponseCode() != 200) {
            try {
                ...
                objArr[url] = e.getMessage();
                form.dispatchErrorOccurredEvent(this, "RequestDirections", ErrorMessages.ERROR_UNABLE_TO_REQUEST_DIRECTIONS, objArr);
                e.printStackTrace();
                connection.disconnect();
            } catch (Throwable th4) {
                th = th4;
        ...
        if (features.size() > 0) {
            ...
            try {
                ...
                objArr2[url] = e.getMessage();
                form2.dispatchErrorOccurredEvent(this, "RequestDirections", ErrorMessages.ERROR_UNABLE_TO_REQUEST_DIRECTIONS, objArr2);
                e.printStackTrace();
                connection.disconnect();
            } catch (Throwable th5) {
                th = th5;
    ...
    private Double[][] getCoordinates(GeoPoint startLocation, GeoPoint endLocation) {
        Double[][] coords = (Double[][]) Array.newInstance((Class<?>) Double.class, 2, 2);
