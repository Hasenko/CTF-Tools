            ...
            switch (literal.symbol) {
                ...
                case 'F':
                    for (int i3 = 0; i3 < literal.size; i3++) {
                        buffer.putFloat(((float[]) values[index2])[i3]);
                    }
                    index2++;
                    break;
                case 'H':
                    for (int i4 = 0; i4 < literal.size; i4++) {
                        buffer.putShort(((short[]) values[index2])[i4]);
                    }
                    index2++;
                    break;
                case 'I':
                    for (int i5 = 0; i5 < literal.size; i5++) {
                        buffer.putInt(((int[]) values[index2])[i5]);
                    }
                    index2++;
                    break;
                case 'L':
                    for (int i6 = 0; i6 < literal.size; i6++) {
                        buffer.putLong(((long[]) values[index2])[i6]);
                    }
                    index2++;
                    break;
                case 'S':
        ...
        for (String token : formatTokens) {
            ...
            switch (symbol) {
                ...
                case 'H':
                    ...
                    continue;
                case 'I':
                    long[] longs = new int[size];
                    for (int i3 = 0; i3 < size; i3++) {
                        longs[i3] = buffer.getInt();
                    }
                    decodedObjects.add(longs);
                    continue;
                case 'L':
                    long[] longs2 = new long[size];
                    for (int i4 = 0; i4 < size; i4++) {
                        longs2[i4] = buffer.getLong();
                    }
                    decodedObjects.add(longs2);
                    continue;
                case 'S':
                    if (sizeSpecified) {
                        throw new IllegalArgumentException("Illegal format string");
