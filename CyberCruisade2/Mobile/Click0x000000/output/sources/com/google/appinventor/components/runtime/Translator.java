...
public final class Translator extends AndroidNonvisibleComponent {
    ...
    public void performRequest(String languageToTranslateTo, String textToTranslate) {
        ...
        try {
            ...
            String srcLanguage3 = this.apiKey;
            byte[] decodedToken = Base58Util.decode(srcLanguage3);
            TranslatorToken.token token = TranslatorToken.token.parseFrom(decodedToken);
            try {
                TranslatorToken.request request = TranslatorToken.request.newBuilder().setToken(token).setTotranslate(textToTranslate).setSourcelanguage(srcLanguage).setTargetlanguage(targetLanguage).m885build();
                ...
                if (connection != null) {
                    try {
                        ...
                        GotTranslation(new StringBuilder().append(responseCode).toString(), translationOrError);
