...
public class DataFile extends FileBase implements DataSource<YailList, Future<YailList>> {
    ...
    protected void afterRead(String result) {
        try {
            if (result.charAt(0) == '{') {
                try {
                    YailList columnsFromJson = JsonUtil.getColumnsFromJson(result);
                    this.columns = columnsFromJson;
                    this.rows = ChartDataSourceUtil.getTranspose(columnsFromJson);
                } catch (JSONException e) {
                    YailList fromCsvTable = CsvUtil.fromCsvTable(result);
                    this.rows = fromCsvTable;
                    this.columns = ChartDataSourceUtil.getTranspose(fromCsvTable);
                }
            } else {
                YailList fromCsvTable2 = CsvUtil.fromCsvTable(result);
                this.rows = fromCsvTable2;
                this.columns = ChartDataSourceUtil.getTranspose(fromCsvTable2);
            }
