...
public class ConstantPool {
    ...
    void rehash() {
        ...
        int i2 = this.count;
        this.hashTab = new CpoolEntry[i2 < 5 ? 101 : i2 * 2];
        CpoolEntry[] cpoolEntryArr = this.pool;
        if (cpoolEntryArr != null) {
            int i3 = cpoolEntryArr.length;
            while (true) {
                i3--;
                if (i3 >= 0) {
                    CpoolEntry entry2 = this.pool[i3];
                    if (entry2 != null) {
    ...
    public CpoolUtf8 addUtf8(String s) {
        ...
        int index = (Integer.MAX_VALUE & h) % cpoolEntryArr.length;
        for (CpoolEntry entry = cpoolEntryArr[index]; entry != null; entry = entry.next) {
    ...
    public ConstantPool(DataInputStream dstr) throws IOException {
        int readUnsignedShort = dstr.readUnsignedShort() - 1;
